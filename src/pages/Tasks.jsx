...
const Tasks = () => {
  ...
  const task = useSelector((state) => state.tasksSlice.tasks);
  const pendingFilter = task.filter((item) => item.status === "pending");
  const runningFilter = task.filter((item) => item.status === "running");
  const completeFilter = task.filter((item) => item.status === "complete");
        ...
        <div className="grid grid-cols-3 gap-5 mt-10">
          ...
          <div className="relative h-[800px] overflow-auto">
            <div className="flex sticky top-0 justify-between bg-[#D3DDF9] p-5 rounded-md mb-3">
              <h1>In Progress</h1>
              <p className="bg-primary text-white w-6 h-6 grid place-content-center rounded-md">
                {runningFilter.length}
              </p>
            </div>
            <div className="space-y-3">
              {runningFilter.map((item) => (
                <TaskCard key={item.id} task={item} />
              ))}
            </div>
          </div>
          <div className="relative h-[800px] overflow-auto">
            <div className="flex sticky top-0 justify-between bg-[#D3DDF9] p-5 rounded-md mb-3">
              <h1>Up Next</h1>
              <p className="bg-primary text-white w-6 h-6 grid place-content-center rounded-md">
                {completeFilter.length}
              </p>
            </div>
            <div className="space-y-3">
              {completeFilter.map((item) => (
                <TaskCard key={item.id} task={item} />
              ))}
            </div>
          </div>
